...
final class FitTrackTests: XCTestCase {
    ...
    func test_bookingAClassRemovesItsCostFromBalance() {
        let mondayYoga = vm.allClasses.first { $0.name == "Yoga" && $0.day == .monday }!
        vm.bookClass(mondayYoga)
        XCTAssertEqual(vm.availableCredits, 10 - mondayYoga.creditCost)
    }
    ...
    func test_purchaseCredits_addsCreditsToBalance_FLAKY() {
        Task { await vm.purchaseCredits(5) }
        Thread.sleep(forTimeInterval: 0.3) // 🚩 guesswork — the real call takes 0.4s
        XCTAssertEqual(vm.availableCredits, 15)
    }

    // MARK: - Exercise 6 (breather): cancelBooking — TODO
